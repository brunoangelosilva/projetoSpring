...
public class AlgamoneyExceptionHandler extends ResponseEntityExceptionHandler {// captura excessões de Resposta de
	...
	@ExceptionHandler({ EmptyResultDataAccessException.class }) // serve para quando uma excessão de
																// EmptyResultDataAccessException ser levantada, vir
																// para esse método.
	public ResponseEntity<Object> handleEmptyResultDataAccessException(EmptyResultDataAccessException ex,
			WebRequest request) {
		// ex de uso da classe : quando tenta excluir um registro que não se encontra
	...
	public ResponseEntity<Object> handleDataIntegrityViolationException(DataIntegrityViolationException ex,
			WebRequest request) {
		// tentou inserir algum informação que gerou uma vioalão de integridade no banco
		// , ex: foreign key
	...
	private List<Erro> criarListaDeErros(BindingResult bindingResult) {
		...
		for (FieldError fieldError : bindingResult.getFieldErrors()) { // fornece todos os erros que aconteceram nos
																		// fields
